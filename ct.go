...
func (c *Client) GetEtherPrice() string {
	coin := new([]Coin)
	_, err := c.sling.New().Get(ether).ReceiveSuccess(&coin)
...
func (c *Client) GetBitcoinPrice() string {
	coin := new([]Coin)
	_, err := c.sling.New().Get(bitcoin).ReceiveSuccess(&coin)
